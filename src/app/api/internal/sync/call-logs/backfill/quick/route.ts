...
export async function POST(request: NextRequest) {
  try {
    ...
    const sliceMinutes = Number(process.env.BACKFILL_CALL_LOGS_SLICE_MINUTES || process.env.BACKFILL_SLICE_MINUTES || '120') || 120
    const pageSize = Number(process.env.BACKFILL_CALL_LOGS_PAGE_SIZE || process.env.BACKFILL_PAGE_SIZE || '200') || 200
    const maxPagesPerSlice = Number(process.env.BACKFILL_CALL_LOGS_MAX_PAGES_PER_SLICE || process.env.BACKFILL_MAX_PAGES_PER_SLICE || '30') || 30
    const localUpsertBatchSize = Number(process.env.SYNC_LOCAL_UPSERT_BATCH_SIZE || '10000') || 10000

    const proxyUrl = new URL('/api/internal/sync/call-logs/backfill', request.url).toString()
