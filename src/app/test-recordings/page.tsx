...
export default function RecordingsPage() {
  ...
  const loadRecordings = async (
    ...
    const activePageNum = options?.pageNum ?? pagination.pageNum
    ...
    try {
      const params = new URLSearchParams({
        pageNum: String(activePageNum),
        pageSize: String(activePageSize),
      })
      if (activeFilters.startTime) params.set('startTime', `${activeFilters.startTime}:00`)
      if (activeFilters.endTime) params.set('endTime', `${activeFilters.endTime}:59`)
      ...
      const recordList =
        ...
        []
      const total = data.total ?? data.count ?? data.data?.total ?? recordList.length

      setRecordings(recordList)
      ...
      setRawTotal(Number(data.rawTotal ?? total ?? 0))
      setDataSourceInfo(
        data.source === 'local-file' ? `数据来源：本地累计文件（${data.sourceFile || 'local-sync'}）` : '数据来源：接口'
      )
  ...
  useEffect(() => {
    loadRecordings()
  }, [pagination.pageNum, pagination.pageSize, appliedFilters])
  ...
  const handleSearch = () => {
    ...
    loadRecordings({ filters: nextApplied, pageNum: 1 })
  }
  ...
  const handleReset = () => {
    ...
    loadRecordings({ filters: defaultFilters, pageNum: 1 })
  }
  ...
  const fetchApiData = async () => {
    ...
    try {
      ...
      setApiRecordings(data.data?.records || data.rows || [])
  ...
  const updateLocalFile = async () => {
    ...
    try {
      ...
      // 重新加载本地数据
      loadRecordings({ filters: appliedFilters, pageNum: 1 })
    } catch (e) {
              ...
              <Table>
                ...
                <TableBody>
                  {apiRecordings.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={apiRecordings.length > 0 ? Object.keys(apiRecordings[0]).length : 1} className="text-center py-10 text-gray-500">
                        暂无数据
                      </TableCell>
                    </TableRow>
                  ) : (
                    apiRecordings.map((recording, idx) => (
                      <TableRow key={String(recording.uuid || recording.id || idx)}>
                        {Object.keys(recording).map((k) => (
                          <TableCell key={`${String(recording.uuid || idx)}-${k}`}>{formatCellValue(k, recording[k])}</TableCell>
                        ))}
                      </TableRow>
                    ))
                  )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {loading
              ? '加载中...'
              : `当前筛选 ${pagination.total} 条（本地累计 ${rawTotal} 条），当前第 ${pagination.pageNum} 页`}
          </CardDescription>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  setPagination((prev) => ({ ...prev, pageNum: Math.max(1, prev.pageNum - 1) }))
                }
              >
                上一页
              </Button>
